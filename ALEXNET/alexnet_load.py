...
def alexnet(input, num_class, transf_params_encoded=['FT', 'FT', 'FT', 'FT', 'FT', 'FT', 'FT', 'NT']):
    ...
    network = conv_2d(input, 12, 11, strides=4, activation='relu', scope='conv1', restore=transf_params[0][0], trainable=transf_params[0][1])
    network = max_pool_2d(network, 3, strides=2, name='maxpool1')
    network = local_response_normalization(network)
    network = conv_2d(network, 32, 5, activation='relu', scope='conv2', restore=transf_params[1][0], trainable=transf_params[1][1])
    network = max_pool_2d(network, 3, strides=2, name='maxpool2')
    network = local_response_normalization(network)
    network = conv_2d(network, 48, 3, activation='relu', scope='3_1', restore=transf_params[2][0], trainable=transf_params[2][1])
    network = conv_2d(network, 48, 3, activation='relu', scope='3_2', restore=transf_params[3][0], trainable=transf_params[3][1])
    network = conv_2d(network, 32, 3, activation='relu', scope='3_3', restore=transf_params[4][0], trainable=transf_params[4][1])
    network = max_pool_2d(network, 3, strides=2, name='maxpool3')
    network = local_response_normalization(network)
    network = fully_connected(network, 512, activation='tanh', scope='fc1', restore=transf_params[5][0], trainable=transf_params[5][1])
    network = dropout(network, keep_prob, name='dropout1')
    network = fully_connected(network, 512, activation='tanh', scope='fc2', restore=transf_params[6][0], trainable=transf_params[6][1])
    network = dropout(network, keep_prob, name='dropout2')
    network = fully_connected(network, num_class, activation='softmax', scope='fc3', restore=transf_params[7][0], trainable=transf_params[7][1])
    return network
